    ...
    const fetchProducts = async () => {
      try {
        const productsData = await API.graphql(graphqlOperation(listProducts, {
          filter: {
            Category: {
              eq: 'Face' // Replace with the actual category name
            }
          }
        }))
    ...
    return (
      <div className="bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-white via-green-100 to-emerald-100">
        <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
          ...
          <div className="bg-white bg-cover mb-10 text-white px-6 py-24 sm:py-32 lg:px-8" style={{ backgroundImage: `url('https://media.giphy.com/media/nwMYEuIqqYUaQ/giphy.gif')` }}>
  <div className="mx-auto max-w-2xl text-center">
    <h2 className="text-4xl font-bold tracking-tight text-white sm:text-6xl">Face</h2>
  </div>
</div>
